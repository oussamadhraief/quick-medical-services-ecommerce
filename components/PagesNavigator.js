...
export default function PagesNavigator(props){
    ...
    return(
        <div className={props.relative ? "relative  h-fit w-fit flex justify-center flex-nowrap py-1 bg-white z-50" : "absolute bottom-0 h-fit w-full flex justify-center flex-nowrap py-1 bg-[rgba(255,255,255,0.9)] z-50 rounded-b-lg"}>
            <button className="px-2 pb-0.5 border-[1px] border-zinc-400 text-light font-bold text-lg mr-1 hover:bg-cool hover:border-cool hover:text-white" onClick={e => {if(pageSelection > 0) setPageSelection(pageSelection -1)}}>&#60;</button>
            {renderPages()}
            <button className="px-2 pb-0.5 border-[1px] border-zinc-400 text-light font-bold text-lg ml-1 hover:bg-cool hover:border-cool hover:text-white" onClick={e => {if(pageSelection != pages -1) setPageSelection(pageSelection + 1)}}>&#62;</button>
        </div>
