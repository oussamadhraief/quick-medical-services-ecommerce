    ...
    return(
        ...
        {categoriesAndSubcategories.map(item => 
                        <ul id={item.category} className='ml-3 my-5'><span onClick={e => {
                            ...
                            {item.subcategories.map(element => <li className="bg-ciel text-sm relative ml-12 transition-[height] expandable h-0 w-7/12 invisible border-b border-white">{element}</li>)}
                        </ul>
                    )}
        </>
    )
