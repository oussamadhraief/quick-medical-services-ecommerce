...
export default function CategoriesNavigator({categoriesAndSubcategories}){
    return(
        ...
        {categoriesAndSubcategories.map(item => 
                        <ul id={item.category} className='ml-3 my-5'><span onClick={e => {
                            ...
                            {item.subcategories.map(element => <li className="bg-zinc-200 text-sm relative ml-12 transition-[height] duration-300 expandable h-0 w-7/12 shadow-lg invisible mt-0 min-w-fit rounded-lg font-medium flex justify-start items-center px-1">
                                <Link href='/'>
                                    <a>{element}</a>
                                </Link>
                                </li>)}
