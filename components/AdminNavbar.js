...
function AdminNavbar(props) {
    ...
    return ( 
    <nav className="adminNavbar h-fit md:h-12 flex flex-wrap md:flex-nowrap items-center w-full  bg-light justify-between px-2 sm:px-10 z-50">
        <button className="h-7 w-7 px-1 py-1 grid place-content-center place-items-center order-1 my-2" onClick={e => handleOpenMenu()}> <Image src={props.open ? closeMenu : menuIcon} width={25} height={25} layout='fixed' /> </button>
        <form onSubmit={e => handleSubmit(e)} className="bg-white mx-auto my-2 pr-1 h-10 md:h-8 flex flex-nowrap w-[99%] sm:w-11/12 md:w-fit items-center rounded-[4px] order-3 md:order-2">
            <div className={categoriesOpen ? "h-10 md:h-8 font-medium outline-none border-0 grid w-40 sm:w-32 py-0.5  bg-zinc-200  rounded-l-[4px] relative" : "relative h-8 font-medium outline-none border-0 grid w-40 sm:w-32 py-0.5  bg-white rounded-l-[4px]"}>
            ...
            <input type="text" name="search" value={searchContext} onChange={e => setSearchContext(e.target.value)} placeholder="Chercher par catégorie" className="w-full md:w-72 px-1 outline-none h-7 border-l border-zinc-300"/>
       ...
       <div className='w-fit h-fit order-2 md:order-3'>
