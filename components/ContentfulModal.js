...
function ContentfulModal({show, onClose, content}) {
    ...
    const handleCloseClick = (e) => {
      ...
      <div className="h-5/6 w-8/12 py-3 sm:py-5 px-5 sm:px-8 bg-white shadow-[0px_3px_26px_5px_rgba(0,0,0,0.6)] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 fixed z-[9999] rounded-xl flex justify-center gap-10">
      <button onClick={(e) => handleCloseClick(e)} className='absolute top-1 right-2 font-medium text-third hover:text-zinc-300'>X</button>
      <div className="border-[1px] border-zinc-200 w-96 mx-auto md:mx-0 flex justify-center h-fit">
            ...
            <div className="w-4/6 h-fit pl-1 grid">
                ...
                <p className="font-medium text-zinc-600 mt-5 text-md">Catégorie:&nbsp;</p>
                <p className="font-medium ml-2">{content.category}</p>
                <p className="font-medium text-zinc-600 mt-5 text-md">Sous-Catégorie:&nbsp;</p>
                <p className="font-medium ml-2">{content.subcategory}</p>
