...
export default function ProductsCarousel({id}) {
    ...
    useEffect(() => {
        ...
        newValue = newValue.map(item => <ScrollableProduct key={item.name} product={item} setShowNotification={setShowNotification} setMessage={setMessage} />)
    ...
    return (
        <div className='scrollable relative w-screen flex flex-nowrap items-center overflow-x-hidden h-fit bg-white'>
            ...
            {renderedArray.length > 0 ? renderedArray : <p className='w-full text-center font-medium'>Aucun resultat trouvé...</p>}
            </div>
