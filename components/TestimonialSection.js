...
export default function TestimonialSection(){
    ...
    const handleScrollRight = () => {
        ...
        setTimeout(() => {
            ...
            let ind3
            let ind4
            if(count + 1 < testimonials.length){ind1 = count + 1}else{ind1 = 0}
            if(ind1 + 1 < testimonials.length){ind2 = ind1 + 1}else{ind2 = 0}
                    ...
                    if(index == 1 || index == 3) return(<div key={index} className={`relative w-[25vw] h-60 shadow-float grayscale blur-[1px] px-10 py-7 group hover:cursor-pointer rounded-md transition-all duration-300`}>
                        ...
                        <div className='absolute w-full h-2 hidden group-hover:block bg-secondary left-0 bottom-0 rounded-b-md'>
                         ...
                         <div className='w-fit mx-auto flex flex-nowrap items-center h-fit gap-3 mt-3'>
                            <div className='w-12 h-12 rounded-full shadow relative' style={{background: `url("https://avatars.dicebear.com/api/personas/${item.author}.svg?mood[]=happy") no-repeat center center`}}>
