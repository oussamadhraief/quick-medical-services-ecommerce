   ...
   return (
        <div className="h-full relative w-full border-2 rounded-lg border-zinc-300 flex justify-center">
            ...
            <div className='min-h-full w-full grid'>
                <div className="min-h-full h-full relative w-full overflow-y-scroll overflow-x-hidden rounded-md pb-8 2xl:pt-10 2xl:px-10  lg:pt-5 lg:px-5 flex justify-evenly flex-wrap">
                {renderedArray.map(item => <AdminProducts key={item.name} image={item.image} name={item.name} reference={item.reference} handleClick={handleEdit} />)}
                </div>
                <PagesNavigator />
            </div>}
        </div>
