   ...
   return (
        <div className="h-full relative w-full pt-2 md:pt-10 flex overflow-y-auto justify-center">
            ...
            <div className='min-h-full w-full grid'>
                ...
                {value.map(item => {
                    return (<AdminProducts key={item.name} image={item.image} name={item.name} reference={item.reference} availability={item.availability} handleLoading={setLoading} archived={item.archived} />)
                })}
                </div>
