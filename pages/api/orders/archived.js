            ...
            try {
                if(session){
                    if(session.user.isAdmin){
                        const NumberOfOrders = await Amazon.countDocuments({status: {$ne :"En cours"}})

                        let Orders
                        
                        if(req.query.page > Math.ceil(NumberOfOrders / 20) -1){
                            
                            Orders = await Amazon.find({status: {$ne :"En cours"}}).sort({createdAt: -1}).limit(20).populate('user')
                            ...
                            Orders = await Amazon.find({status: {$ne :"En cours"}}).sort({createdAt: -1}).skip(req.query.page* 20).limit( 20).populate('user')
